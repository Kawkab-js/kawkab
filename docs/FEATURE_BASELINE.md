...
- **`worker_threads`:** one OS thread per worker with an isolated QuickJS runtime; `postMessage` / `parentPort` payloads are **JSON-serializable** only in this baseline (see **Remaining vs Node** in `NODE_COMPATIBILITY.md`).
